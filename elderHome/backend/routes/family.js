...
router.post('/add', async (ctx, next) => {
    ...
    let familyName = ctx.request.body.content.familyName;
    let relations = ctx.request.body.content.relations;
    let telephone = ctx.request.body.content.telephone;
...
router.post('/update', async (ctx, next) => {
    ...
    try {
        const [rows, fileds] = await connection.query(
            `UPDATE \`family\` SET \`loginName\` = '${loginName}', \`familyName\` = '${familyName}', \`relations\` = '${relations}', \`telephone\` = '${telephone}'  
            WHERE loginName = '${loginName}' AND familyName = '${familyName}'`,
        );
        ctx.body = 'ok';
    } finally {
        connection.release();
